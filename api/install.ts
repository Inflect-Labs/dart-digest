...
REPO="Inflect-Labs/dart-digest"
INSTALL_DIR="$HOME/.dart-digest"
...
# ── download & extract ───────────────────────────────────────────────────────
TMP_DIR="$(mktemp -d /tmp/dtd-install.XXXXXX)"
trap 'rm -rf "$TMP_DIR"' EXIT

echo "Downloading $TARBALL_URL ..."
curl -fsSL "$TARBALL_URL" -o "$TMP_DIR/dtd.tar.gz" || {
  echo "Error: download failed." >&2
  exit 1
}

mkdir -p "$TMP_DIR/extracted"
tar -xzf "$TMP_DIR/dtd.tar.gz" -C "$TMP_DIR/extracted"
...
# ── symlink dtd ─────────────────────────────────────────────────────────────
chmod +x "$INSTALL_DIR/bin/dtd"

link_bin() {
  ln -sf "$INSTALL_DIR/bin/dtd" "$BIN_DIR/dtd"
}
...
if link_bin 2>/dev/null; then
  ...
  sudo ln -sf "$INSTALL_DIR/bin/dtd" "$BIN_DIR/dtd"
fi
