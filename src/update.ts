...
const REPO = "Inflect-Labs/dart-digest";
const INSTALL_URL = "https://dart-digest-inflectlabs.vercel.app/install";
...
export async function uninstall(): Promise<void> {
  ...
  let binaryPath = "/usr/local/bin/dtd";
  try {
    binaryPath = execSync("which dtd", { encoding: "utf-8" }).trim();
  } catch {
    // fall back to default
  }

  const installDir = join(homedir(), ".dart-digest");

  const confirmed = await confirm({
    message: `This will remove dtd from ${binaryPath} and ${installDir}. Continue?`,
    default: false,
  });
  ...
  console.log("dtd uninstalled successfully.");
}
